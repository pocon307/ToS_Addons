function GUILD_AUTHO_POPUP_DO_OPEN(frame, ctrl, argStr, argNum)
	...
	for i = 1 , numCount do
		...
		for j = 0 , plist:Count() - 1 do
			...
			if (showOnlyConnected == 0) or (partyMemberInfo:GetMapID() > 0) then
				...
				if (i == 1) then
					local checkbox_line = bg_checkgroup:CreateOrGetControl('labelline', 'LINE_'..connectionCount, 4, subY + checkbox_member:GetHeight() - 2, maxWndW - 25, 10);
					checkbox_line:SetSkinName('labelline_def_2');
				end
				
				connectionCount = connectionCount + 1;
				subY = subY + checkbox_member:GetHeight() +7;		
			end
		end			
	...
	bg_checkgroup:Resize(maxWndW - 17, maxWndH - 10);	
	bg_title_key:Resize(maxWndW - 20, bg_title_key:GetHeight());
end
